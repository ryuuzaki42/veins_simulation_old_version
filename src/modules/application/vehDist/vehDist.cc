...
WaveShortMessage* vehDist::generateMessage(){
    ...
    //calcuting the distance between the car and the RSU
    Coord posCar = traci->getCurrentPosition();
    //Coord posRSU = Coord(240, 1000, 3);
    Coord posRSU = wsm->getTargetPos();
    cout<<"RSU position: " << wsm->getTargetPos() << endl;
    double distance = 950;
    cout<<findHost()->getFullName()<<" before measure the distance at " << simTime()<<endl;
    //distance = traci->commandDistanceRequest(posCar, posRSU, false);
    cout<<findHost()->getFullName()<<" after measure the distance "<< distance<< " at "<<simTime()<<endl;
    ...
    wsm->setGlobalMessageIdentificaton((to_string(vehDist::messageId) + to_string(MACToInteger())).c_str());
...
void vehDist::printContextLocalMessageBuffer() {
    if (contextLocalMessageBuffer.empty()) {
        ...
        cout << "Printing the contextLocalMessageBuffer from " << findHost()->getFullName() << "(" << MACToInteger() <<"):" << endl;
        map<string, WaveShortMessage>::iterator it;
        for(it = contextLocalMessageBuffer.begin(); it != contextLocalMessageBuffer.end(); it++) {
...
void vehDist::printBeaconNeighbors() {
    if (beaconNeighbors.empty()) {
        ...
        cout << "Printing the beaconNeighbors from " << findHost()->getFullName() << "(" << MACToInteger() <<"):" << endl;
...
void vehDist::onData(WaveShortMessage* wsm) {
    ...
    if((wsm->getRecipientAddress() == 268435455)||(wsm->getRecipientAddress() == MACToInteger())){
        ...
        if((contextLocalMessageBuffer.empty())||(search == contextLocalMessageBuffer.end())){
            ...
            wsmdup->setSenderAddress(MACToInteger());
