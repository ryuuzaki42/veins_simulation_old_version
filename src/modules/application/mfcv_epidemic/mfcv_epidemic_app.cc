...
#include "application/mfcv_epidemic/mfcv_epidemic_app.h"
#include "mobility/traci/TraCIColor.h"

using Veins::TraCIMobility;
using Veins::TraCIMobilityAccess;

using Veins::mfcv_epidemic_app;

Define_Module(Veins::mfcv_epidemic_app);

const simsignalwrap_t mfcv_epidemic_app::mobilityStateChangedSignal = simsignalwrap_t(MIXIM_SIGNAL_MOBILITY_CHANGE_NAME);

void mfcv_epidemic_app::initialize(int stage) {
    ...
    if (stage == 0) {
        ...
        if (debug) std::cout << "mfcv_epidemic_app initialized with testNumber=" << testNumber << std::endl;
    }
}

void mfcv_epidemic_app::finish() {
}

void mfcv_epidemic_app::handleSelfMsg(cMessage *msg) {
}

void mfcv_epidemic_app::handleLowerMsg(cMessage* msg) {
    delete msg;
}

void mfcv_epidemic_app::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj) {
    if (signalID == mobilityStateChangedSignal) {
        handlePositionUpdate();
    }
}

void mfcv_epidemic_app::handlePositionUpdate() {
