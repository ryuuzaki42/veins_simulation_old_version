...
WaveShortMessage* mfcv_epidemic_rsu::prepareWSM_mfcv_epidemic(std::string name, int lengthBits, t_channel channel, int priority, unsigned int rcvId, int serial) {
    cout << "In prepareWSM_mfcv_epidemic" << endl;
    ...
    cout << "In prepareWSM_mfcv_epidemic2" << endl;

    // test Jonh
    wsm->setHeading(getAngleDegrees());
...
unsigned int mfcv_epidemic_rsu::getAngleDegrees(){
   // RSU return angle 0
   return 0;
}

void mfcv_epidemic_rsu::handleSelfMsg(cMessage* msg) {
    switch (msg->getKind()) {
