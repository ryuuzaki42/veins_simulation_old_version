...
#include "application/epidemic/epidemic_app.h"
#include "mobility/traci/TraCIColor.h"

using Veins::TraCIMobility;
using Veins::TraCIMobilityAccess;

using Veins::epidemic_app;

Define_Module(Veins::epidemic_app);

const simsignalwrap_t epidemic_app::mobilityStateChangedSignal = simsignalwrap_t(MIXIM_SIGNAL_MOBILITY_CHANGE_NAME);

void epidemic_app::initialize(int stage) {
	...
	if (stage == 0) {
		...
		if (debug) std::cout << "epidemic_app initialized with testNumber=" << testNumber << std::endl;
	}
}

void epidemic_app::finish() {
}

void epidemic_app::handleSelfMsg(cMessage *msg) {
}

void epidemic_app::handleLowerMsg(cMessage* msg) {
	delete msg;
}

void epidemic_app::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj) {
	if (signalID == mobilityStateChangedSignal) {
		handlePositionUpdate();
	}
}

void epidemic_app::handlePositionUpdate() {
