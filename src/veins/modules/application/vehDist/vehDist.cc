...
void vehDist::vehInitializeVariables() {
    ...
    if (myId == 0) { // Vehicle must be the first to generate messages, so your offset is 0;
        vehDist::countMeetPBigerT = vehDist::countMsgPacketSend = vehDist::msgBufferUseGeneral = 0;
        vehDist::msgDroppedbyTTL = vehDist::msgDroppedbyHop = vehDist::msgDroppedbyBuffer = 0;
        vehDist::countMesssageDrop = vehDist::countTwoCategoryN = vehDist::countMeetN = vehDist::countVehicleAll = 0;
...
string vehDist::neighborWithShortestDistanceToTarge(string key) {
    ...
    for (itBeaconNeighbors = beaconStatusNeighbors.begin(); itBeaconNeighbors != beaconStatusNeighbors.end(); itBeaconNeighbors++) {
        if (!messagesSendLog.empty() && (messagesSendLog[key].find(itBeaconNeighbors->first) != std::string::npos)){
            cout << source << " has been send a message " << key << " to " << itBeaconNeighbors->first << endl;
        } else {
    ...
    if (!vehShortestDistanceToTarget.empty()) {
        ...
        cout << "    Id: " << source << endl;
        cout << "    Message Id: " << messagesBuffer[key].getGlobalMessageIdentificaton() << endl;
        cout << "    Category: " << vehCategory << endl;
        cout << "    Distance: " << traci->getDistance(curPosition, messagesBuffer[key].getTargetPos(), false) << endl;
...
void vehDist::printCountBeaconMessagesDrop() {
    ...
    if (vehDist::numVehicles.size() == 1) {
        ...
        myfile << "Exp: " << expNumber << " ### Count meet with real difference: " << vehDist::countMeetPBigerT << endl;
...
void vehDist::sendMessageNeighborsTarget(string beaconSource) {
    ...
    while(countMessage > 0) {
        if (beaconSource.compare(itMessage->second.getTarget()) == 0) {
            ...
            cout << source <<" sending message: " << idMessage << " to: " << beaconSource << " and removing" << endl;
    ...
    if (myId == 0) { // if true, some vehicle has (in past) selected the vehicle to generate messages
        if (simTime() <= vehDist::timeLimitGenerateBeaconMessage) {
            ...
            for (unsigned short int i = 0; i < countGenerateBeaconMessage;) { // select <countGenerateBeaconMessage> distinct vehicle to generate messages
                ...
                stringTmp = numVehicles[vehSelected];

                auto it = find(vehDist::vehGenerateMessage.begin(), vehDist::vehGenerateMessage.end(), stringTmp);
                if (it == vehDist::vehGenerateMessage.end()) {
                    vehDist::vehGenerateMessage.push_back(stringTmp);
                    cout << endl << source << " selected the " << stringTmp << " to generate a message at: " << simTime() << endl;
                    myfile << source << " selected the " << stringTmp << " to generate a message at: " << simTime() << endl;
                    i++;
                }
            }
            myfile.close();
