...
void service_discovery::deterministQuery() {
...
void service_discovery::onBeacon(WaveShortMessage* wsm) {
...
void service_discovery::handleSelfMsg(cMessage* msg) {
              switch (msg->getKind()) {
...
void service_discovery::contService(const char* cat){
        std::string op1 = cat;
...
void service_discovery::sendMessage(const char* serv) {
    sentMessage = true;
...
void service_discovery::insertService(const char* serv){
...
void service_discovery::insertService2(const char* serv){
       bool exist=false;
       int x;
       std::string op3 = serv;
...
void service_discovery::queryService(const char* serv){
    std::string op1=serv;
...
void service_discovery::deleteService(const char* serv){
    bool exist=false;
    int x,p;
    for (int i=0; i< ns; i++) {
        x= strcmp (service[i].c_str(), serv);
...
void service_discovery::answerQuery(const char* serv){
    double speed=mobility->getSpeed();
    if (receiver==myId)
    {
...
void service_discovery::cancelQuery(const char* serv){
